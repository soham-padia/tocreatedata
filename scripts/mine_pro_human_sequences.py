...
def parse_args() -> SequenceMiningConfig:
    parser = argparse.ArgumentParser(
        description="Exhaustively score short lexicon sequences by mechanistic projection onto a saved direction vector."
    )
...
def main() -> None:
    ...
    for batch in batched(
        ...
        first_indices = [item[2] for item in batch]
        ...
        if total_sequences % max(1, config.batch_size * 100) == 0:
            ...
            print(
                f"scored_sequences={total_sequences} retained={len(retained)} "
