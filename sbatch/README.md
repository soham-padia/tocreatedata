...
- For mechanistic pro-human dataset mining, submit `sbatch sbatch/mine_mechanistic_dataset.sbatch` with `DIRECTION_TENSORS` pointing at a saved `directions.pt`. It will score candidate strings by activation-space projection and keep the top `KEEP_TOP_K` results under `outputs/mechanistic_dataset/`.
- For exhaustive pro-human sequence mining over the controlled lexicon, run `bash run_pro_human_mechanistic_hpc.sh`. It submits parallel shard jobs over lengths `1..5`, retains a larger frontier per shard, and automatically merges a final global top-K set under `outputs/mechanistic_dataset/`.
